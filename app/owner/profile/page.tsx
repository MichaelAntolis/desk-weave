...
export default function OwnerProfilePage() {
  ...
  return (
    <div className="flex flex-col h-full bg-surface">
      <header className="hidden md:flex flex-row bg-surface-container-lowest/80 backdrop-blur-xl sticky top-0 w-full z-30 px-4 md:px-8 py-4 md:py-6 justify-between items-center border-b border-outline/10">
        <div>
          <h2 className="text-xl md:text-2xl font-extrabold text-primary tracking-tight font-headline">
            Profil Pemilik Ruangan
          </h2>
          <div className="hidden md:flex items-center gap-2 mt-1">
            ...
            <span className="text-xs font-label uppercase tracking-widest text-primary">Profile</span>
          </div>
        </div>
        <div className="flex gap-2 items-center">
          <NotificationBell baseUrl="/owner" />
          <div className="ml-4 pl-4 border-l border-outline/20 flex flex-row items-center gap-3">
            <div className="text-right">
              <p className="text-sm font-bold text-primary">{displayName}</p>
              <p className="text-[10px] text-on-surface-variant uppercase tracking-wider">Owner Account</p>
            </div>
            <ProfileMenu role="owner" />
          </div>
        </div>
      </header>
      ...
      <section className="px-4 md:px-8 py-6 md:py-8 flex-1">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-20 md:pb-0 font-body">
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-surface-container-lowest rounded-[2rem] p-8 border border-outline/10 shadow-sm text-center relative overflow-hidden group">
              <div className="relative z-10">
                 <div className="w-28 h-28 mx-auto rounded-[2.5rem] bg-surface-container flex items-center justify-center mb-6 relative cursor-pointer overflow-hidden ring-4 ring-primary/5 shadow-2xl transition-all hover:scale-105 active:scale-95" onClick={() => fileInputRef.current?.click()}>
                   {uploading ? (
                 ...
                 <div className="mt-6 inline-flex items-center gap-1.5 px-5 py-2.5 bg-secondary/10 text-secondary rounded-2xl border border-secondary/20 shadow-sm">
                   ...
                   <span className="text-[10px] font-black uppercase tracking-[0.2em]">Verified Owner</span>
                 </div>
              </div>
            ...
            <div className="bg-surface-container-lowest rounded-[2rem] p-8 border border-outline/10 shadow-sm space-y-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <p className="text-[10px] text-on-surface-variant uppercase tracking-widest font-black mb-0.5">Nomor Telepon</p>
                  <p className="font-bold text-on-surface">{profile?.nomor_telepon || "N/A"}</p>
                </div>
              </div>
              <div className="flex items-start gap-4">
                <div className="w-10 h-10 rounded-xl bg-surface-container flex items-center justify-center text-on-surface-variant"><Building className="w-5 h-5" /></div>
                <div>
                  <p className="text-[10px] text-on-surface-variant uppercase tracking-widest font-black mb-0.5">Nama Bisnis</p>
                  <p className="font-bold text-on-surface">{profile?.nama_bisnis || "Individu"}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-surface-container-lowest rounded-[2.5rem] p-8 md:p-10 border border-outline/10 shadow-sm">
              <div className="flex items-center gap-4 border-b border-outline/5 pb-6 mb-8">
                 <div className="w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center text-primary"><UserCircle className="w-6 h-6" /></div>
                 <h3 className="text-xl font-black font-headline text-on-surface tracking-tight">Detail Informasi</h3>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black text-on-surface-variant font-label uppercase tracking-[0.2em] px-1">Email Aktif</Label>
                  ...
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black text-on-surface-variant font-label uppercase tracking-[0.2em] px-1">Nomor Telepon</Label>
                    <div className="relative group">
                  ...
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black text-on-surface-variant font-label uppercase tracking-[0.2em] px-1">Nama Bisnis / Brand</Label>
                    <div className="relative group">
                ...
                {success && (
                  <div className="p-4 bg-green-500/10 border-2 border-green-500/20 rounded-2xl text-green-600 text-sm font-bold flex items-center gap-2 animate-in slide-in-from-top-2">
                     <ShieldCheck className="w-4 h-4" /> Profil berhasil diperbarui!
                  </div>
                )}
                ...
                <div className="flex justify-end pt-4">
                  <Button type="submit" disabled={saving} className="h-14 px-10 rounded-2xl gradient-primary text-white font-black text-xs uppercase tracking-widest shadow-xl shadow-primary/30 active:scale-95 transition-all">
                    {saving ? <><Loader2 className="w-4 h-4 animate-spin mr-3" /> Processing</> : "Simpan Informasi"}
                  </Button>
                </div>
              </form>
            </div>

            <div className="bg-surface-container-lowest rounded-[2.5rem] p-8 md:p-10 border border-outline/10 shadow-sm relative overflow-hidden">
              <div className="flex items-center gap-4 border-b border-outline/5 pb-6 mb-8">
                 ...
                 <h3 className="text-xl font-black font-headline text-on-surface tracking-tight">Privasi &amp; Akses</h3>
              </div>
              <form className="space-y-8 relative z-10">
                <div className="space-y-4 max-w-md">
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black text-on-surface-variant font-label uppercase tracking-[0.2em] px-1">Password Baru</Label>
                    <div className="relative group">
                ...
                <div className="flex justify-start pt-4">
                  <Button type="button" variant="outline" className="h-14 px-8 rounded-2xl border-2 border-primary/20 text-primary hover:bg-primary/5 font-black text-xs uppercase tracking-widest active:scale-95 transition-all">
                    Ubah Password Akun
                  </Button>
                </div>
              </form>
              <div className="absolute bottom-0 right-0 w-40 h-40 bg-on-surface/[0.02] translate-y-1/2 translate-x-1/2 rounded-full" />
            </div>
          </div>
        </div>
      </section>
